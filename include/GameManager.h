...
public:
    ...
    // 在指定关卡和地图上操作对象
    void AddObject(const LevelID& levelId, const MapID& mapId, const Location& loc, AutoGameObject obj);     
    void RemoveObject(const LevelID& levelId, const MapID& mapId, const Location& loc);                     
    void MoveObject(const LevelID& levelId, const MapID& mapId, const Location& from, const Location& to); 
...
private:
    ...
    GameLevel* _currentLevel; // 当前关卡
    GameMap* _currentMap;//当前地图
};
